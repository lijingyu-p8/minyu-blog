...
- 集群broker
  ...
  ![broker-cluster-1](/RocketMQ/01/broker-cluster-1.png)

- Broker功能模块划分：

  ![image-20210818124848865](/RocketMQ/01/broker-1.png)
...
- 消费者组中Consumer的数量应该小于等于订阅Topic的Queue数量。如果超出Queue数量，则多出的Consumer将不能消费消息。
  ...
  ![image-20210817124317733](/RocketMQ/01/负载均衡-1.png)
...
- 主要由Producer、NameServer、Broker、Consumer组成。
  ...
  ![image-20210817123524555](/RocketMQ/01/系统架构-1.png)
...
![](/RocketMQ/01/消息的存储机制-1.png)

Producer发送数据到Broker，数据会被记录到commitLog里，并且会更新当前offset消费记录。

### 2、文件列表

![image-20210820123845958](/RocketMQ/01/消息的存储机制-文件列表-1.png)
...
- 数据存储真正的物理单元。一个commitLog大小为1g，commitLog目录中存放着很多的mappedFile文件，当前Broker中的所有消息都是落盘到这些mappedFile文件中的。mappedFile文件大小为1G（小于等于1G），文件名由20位十进制数构成，表示当前文件的第一条消息的起始位移偏移量。
  ...
  ![image-20210823123910891](/RocketMQ/01/commitlog-offset-1.png)
...
- consumequeue文件名也由20位数字构成，表示当前文件的第一个索引条目的起始位移偏移量。与mappedFile文件名不同的是，其后续文件名是固定的。因为consumequeue文件大小是固定不变
  ...
  ![image-20210823124249332](/RocketMQ/01/consumequeue文件目录结构-1.png)
...
- 每个consumequeue文件可以包含30w个索引条目，每个索引条目包含了三个消息重要属性：消息在mappedFile文件中的偏移量CommitLog Offset、消息长度、消息Tag的hashcode值。这三个属性占20个字节，所以每个文件的大小是固定的30w * 20字节
  ...
  ![image-20210823124800293](/RocketMQ/01/consumequeue-1.png)
...
- 索引条目结构
  ...
  ![image-20210824161705389](/RocketMQ/01/index-file-1.png)

  indexHeader固定40个字节，其中存放着如下数据：

  ![image-20210824161812265](/RocketMQ/01/index-header-1.png)
  ...
  ![image-20210824162519265](/RocketMQ/01/index-header-2.png)
  ...
  ![image-20210824163007260](/RocketMQ/01/index-header-3.png)
...
- ![image-20210826124002736](/RocketMQ/01/集群消费-1.png)
...
- ![image-20210826123516015](/RocketMQ/01/广播模式-1.png)
...
- 0:8,1:7,2:7,3:8----代表的含义是queueid为0的队列，消费进度为8，queueId为1的队列，消费进度为7。以此类推。
  ...
  ![image-20210826124539493](/RocketMQ/01/消费进度-1.png)
...
- queue比client多的情况下：
  ...
  ![](/RocketMQ/01/平均分配-1.png)

- queue比client少的情况下：

  ![](/RocketMQ/01/平均分配-2.png)
...
- queue比client多的情况下：
  ...
  ![](/RocketMQ/01/环形-1.png)

- queue比client少的情况下：

  ![](/RocketMQ/01/环形-2.png)
...
- ![image-20210830124003321](/RocketMQ/01/一致性hash-1.png)
...
- ![image-20210830124148045](/RocketMQ/01/同机房策略-1.png)
...
- offset远程管理模式
  ...
  <!-- <p color="red">集群模式下offset采用远程管理模式，主要是为了保证Rebalance机制。</p> -->
...
- ![image-20210831125140108](/RocketMQ/01/offset-重试存储.png)
...
- 同步发送消息
  ...
  ![image-20210903123028462](/RocketMQ/01/同步发送消息-1.png)

- 异步发送消息

  异步发送消息是指，Producer发出消息后无需等待MQ返回ACK，直接发送下⼀条消息。该方式的消息可靠性可以得到保障，消息发送效率也可以。

  ![image-20210903123120849](/RocketMQ/01/异步发送消息-1.png)
...
- 为什么需要顺序消息
  ...
  ![image-20210903125114966](/RocketMQ/01/顺序消息-1.png)

  ![image-20210903125201965](/RocketMQ/01/顺序消息-2.png)
...
- 顺序消息解决
  ...
  ![image-20210903125438987](/RocketMQ/01/顺序消息-3.png)
...
- 延时消息的延迟时长不支持随意时长的延迟，是通过特定的延迟等级来指定的。延时等级定义在RocketMQ服务端的MessageStoreConfig类中的如下变量中：
  ...
  ![image-20210908125158489](/RocketMQ/01/延时消息-1.png)
...
- 延时消息的原理
  ...
  ![image-20210908125547753](/RocketMQ/01/延时消息-原理-1.png)
